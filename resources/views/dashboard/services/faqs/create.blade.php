...
@section('title', __('Create Service FAQ'))
...
@section('breadcrumb-items')
    ...
    <li class="breadcrumb-item"><a href="{{ route('dashboard.service-faqs.index', $service->id) }}">{{ __('Service FAQs') }}</a></li>
    ...
    <div class="container-xxl flex-grow-1 container-p-y">
        <div class="card mb-6">
            ...
            <div class="card-body pt-4">
                <form method="POST" action="{{ route('dashboard.service-faqs.store', $service->id) }}" enctype="multipart/form-data">
                    @csrf

                    <div class="row p-5">
                        <h3>{{ __('Add New Service FAQ') }}</h3>
                        ...
                        <div class="mb-4 col-md-12">
                            ...
                            <textarea class="form-control @error('answer') is-invalid @enderror" id="answer" name="answer"
                                placeholder="{{ __('Enter answer') }}" cols="30" rows="10">{{ old('answer') }}</textarea>
                            @error('answer')
                                <span class="invalid-feedback" role="alert">
                                    <strong>{{ $message }}</strong>
                                </span>
                    ...
                    <div class="mt-2">
                        <button type="submit" class="btn btn-primary me-3">{{ __('Add Service FAQ') }}</button>
                    </div>
                </form>
            </div>
            <!-- /Account -->
